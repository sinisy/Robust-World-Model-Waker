...
class Environment(dm_env.Environment):
  ...
  def reset(self, env_params=None):
    ...
    self._step_count = 0
    self._physics = self._task.update_physics(self._physics, env_params)
    with self._physics.reset_context():
      self._task.initialize_episode(self._physics, env_params)

    observation = self._task.get_observation(self._physics)
    if self._task.env_params is not None:
      observation["env_params"] = self._task.env_params
      print(f"Env params set to {self._task.env_params}")
  ...
  def env_spec(self):
    """May return a specification for the high and low env params"""
    return self._task.env_spec()
